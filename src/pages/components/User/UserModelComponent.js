...
const UserModelComponent = ({ open, setOpen, isAddButton, rowData, setRefreshData, trackno, isCommissionUpdate, setIsCommissionUpdate }) => {
    //basic information
    ...
    const handleSubmit = (e) => {
        ...
        if (isAddButton) {
            if (!user_email || !firstname || !lastname || !user_password || !user_dealership || !userRole) {
                ...
                <DialogContent>
                    <Grid item xs={12}>
                        <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                            <Grid item xs={4} style={{ display: isAddButton ? "block" : isCommissionUpdate ? "none" : "block" }}>
                                <FormControl fullWidth >
                                    ...
                                    <Select
                                        ...
                                        disabled={userType === 'dealership' || userType == 'manager' || isCommissionUpdate}
                            ...
                            <Grid item xs={4} style={{ display: isAddButton ? "block" : isCommissionUpdate ? "block" : "none" }}>
                                <FormControl fullWidth>
                                    <Typography variant="subtitle2" gutterBottom>
                                        Commission %
                                    </Typography>
                                    <TextField
                                        value={commissionPerc}
                                        margin="dense"
                                        id="outlined-basic"
                                        variant="outlined"
                                        required
                                        disabled={!(isAddButton || isCommissionUpdate)}
                                        onChange={(e) => { setCommissionPerc(e.target.value) }}
                                        sx={{
                                            '& .MuiOutlinedInput-root': {
                                                '& fieldset': {
                                                    borderColor: 'lightgray', // Default border color
                            ...
                            <Grid item xs={2} style={{ display: isAddButton ? "block" : "none" }}>
                                <FormControl fullWidth>
                                    <Typography variant="subtitle2" gutterBottom>
                                        Password
                                    </Typography>
                            ...
                            <Grid item xs={2} style={{ display: isAddButton ? "none" : isCommissionUpdate ? "none" : "block" }}>
                                <FormControl fullWidth>
                                    <Typography variant="subtitle2" gutterBottom>
                                        Is Relationship Manager?
                                    </Typography>
                                    <Checkbox
                                        checked={isRelationshipManager == 1}
                                        onChange={(e) => setIsRelationshipManager(e.target.checked ? 1 : 0)}
                                        sx={{
