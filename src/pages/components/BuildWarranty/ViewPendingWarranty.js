...
const URL = "./dealership/getPending";

function ViewPendingWarranty() {
    ...
    // useEffect that runs after the `trigger` changes
    useEffect(() => {
        if (trigger) {
        ...
        loadDataVin();
        loadDealership();
    }, [trigger]); // Dependency on `trigger` state
    ...
    const columns = [
        ...
        { field: "user", headerName: "Dealer Salesperson", width: 200 },
        // { field: "id", headerName: "Internal ID", width: 100 },
        {
            ...
            getActions: (params) => {
                return userType == "admin" ? [
                    ...
                    <CopyData selectedRow={params.row} />,
                    <EditData selectedRow={params.row} />,
                    <DeleteData selectedRow={params.row} />,
    ...
    const loadDataVin = async () => {
        try {
            const response = await axios.get("https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVinValues/5UXWX7C5*BA?format=json&modelyear=2011");
                ...
                <DeleteIcon
                    onClick={() => {
                        ...
                        if (isConfirmed) {
                            ...
                            const data = { id: props.selectedRow.id, user: userName, VINNO: props.selectedRow.vinNo, dealership: props.selectedRow.dealership };
                ...
                <Stack spacing={3}>
                    <Stack direction="row" spacing={3}>
                        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
                            ...
                            >
                                <TaskIcon sx={{ fontSize: '20px' }} /> &nbsp;Pending warranty
                            </Typography> */}
                ...
                <DataGrid
                    ...
                    sx={{
                        '& .MuiDataGrid-columnHeaderTitle': {
                            ...
                            fontWeight: 'bold',  // Optional: Make column header text bold
                        }
                    }}
...
export default ViewPendingWarranty;
