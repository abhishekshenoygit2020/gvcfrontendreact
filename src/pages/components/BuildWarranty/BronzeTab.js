...
import BronzeA from './PricingPremiumGCC/BronzeA';
...
export default function BronzeTab({ packagesTypes, setPackagesType, productIndex, setProductIndex, setProductName, setProductCost, handleNext }) {
  const theme = useTheme();
  const [value, setValue] = React.useState(packagesTypes < 1 ? packagesTypes : 0);
  ...
  return (
    <Box sx={{ bgcolor: 'background.paper', width: '100%' }}> {/* Full-width */}
      ...
      <TabPanel value={value} index={0} dir={theme.direction}>     
        <BronzeA productIndex={productIndex} setProductIndex={setProductIndex} setProductName={setProductName} setProductCost={setProductCost} handleNext={handleNext} />
