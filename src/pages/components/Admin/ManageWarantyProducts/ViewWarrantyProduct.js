...
function ViewWarrantyProduct() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("warranties");

  const inputRef = useRef(); // maintain focus
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        "http://localhost:3006/api/product/updateAllPrices",
        {
      ...
      if (response.data?.success === 1) {
        setMessage(response.data.message || "Prices updated successfully!");
        setSeverity("success");
        loadData(); // refresh data
      } else {
        setMessage(response.data?.message || "Failed to update prices!");
        setSeverity("error");
      }
      ...
      <Tooltip title="Edit">
        <EditIcon
          ...
          onClick={(e) => {
            ...
            navigate("/WarrantyProducts", {
              state: { type: "update", value: props.selectedRow },
            });
          }}
        />
      </Tooltip>
      ...
      <Tooltip title="Customize Pricing">
        <ManageAccountsIcon
          ...
          onClick={(e) => {
            ...
            console.log(props.selectedRow);
            navigate("/WarrantyProducts", {
              state: { type: "dealershipCustom", value: props.selectedRow },
            });
          }}
        />
      </Tooltip>
      ...
      <Tooltip title="Salesrep Custom Product">
        <ManageAccountsIcon
          ...
          onClick={(e) => {
            ...
            console.log(props.selectedRow);
            navigate("/WarrantyProducts", {
              state: { type: "salesrepCustom", value: props.selectedRow },
            });
          }}
        />
      </Tooltip>
            ...
            Toolbar: () => (
              <Box
                sx={{
                  padding: 2,
                  display: "flex",
                  justifyContent: "space-between", // Typography left, form right
              ...
              >
                ...
                {/* Right side: dropdown + input + button */}
                <Box sx={{ display: "flex", alignItems: "stretch", gap: 0 }}>
                  ...
                  <TextField
                    ...
                    sx={{
                      ...
                      "& .MuiOutlinedInput-root": {
                        ...
                        "&:hover fieldset": { borderColor: "#0d2365" },
                  ...
                  >
                    <MenuItem value="warranties">Warranties</MenuItem>
                    <MenuItem value="gap_protection">Gap Protection</MenuItem>
                  </TextField>

                  {/* Increment input */}
                  <TextField
                    placeholder="Update Products Price"
                    value={increment}
...
export default ViewWarrantyProduct;