...
function AddDealership() {
  ...
  const omvicno = ApplicationStore().getStorage('omvic_no') == null ? "" : ApplicationStore().getStorage('omvic_no');
  ...
  const handleSubmit = async (e) => {
    ...
    // Input Validation
    if (!currentPassword || !newPassword || !verifyPassword  || !ovmic_no) {
    ...
    const method = "POST";
    const data = { oldPassword: currentPassword, newPassword, email: user_email, initialLogin: 0 , ovmic_no: ovmic_no};
    const mainURL = URL;
        ...
        <Container maxWidth="sm" >
          ...
          <form >
            <Box component="main" sx={{ flexGrow: 1, p: 3, border: '1px solid rgb(229 231 235 / 99%);', borderRadius: '8px' }}>
                  ...
                  <OutlinedInput
                    ...
                    endAdornment={
                      <InputAdornment position="end">
                        <IconButton
                          aria-label={showPasswordC ? 'hide the password' : 'display the password'}
              ...
              <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                <FormControl fullWidth>
                  {/* <Typography variant="subtitle2">OMVIC NO</Typography> */}
                  <TextField
                    value={ovmic_no}
